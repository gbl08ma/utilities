...
int isRTCunadjusted() {
   ...
   * this function finds out if the clock is unadjusted if its date/time is set to a
   * previous date/time than 1st Jan 2013 00:00:00.000
   *   (because that date has passed on all timezones and of course no adjusted clock will have it).
   * this function is only meant to be useful for detecting if the RTC performed a reset,
   * e.g. after taking out the batteries.
   * in that case, the RTC usually resets to some date around 2010, so this trick will work.
   */
...
int tickDifferenceToMilliseconds(int prevticks, int newticks) {
  // get difference in milliseconds between two amounts of ticks
...
int isDateValid(int y, int m, int d) {
  if(y < LOWEST_SUPPORTED_YEAR || y > HIGHEST_SUPPORTED_YEAR)
    return 0; // otherwise we will have problems with calendar events and the like
  if (m > 12 || m <= 0 || d > getMonthDaysWithLeap(m, y) || d <= 0)
    return 0;
  else
    return 1;
}
